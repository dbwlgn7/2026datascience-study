...
# -----------------------------------------------------------------------------
# 2. 한국 시각(KST: UTC+9) 기준 '어제' 날짜 계산
# Streamlit Cloud 서버는 해외(UTC) 기준시를 사용하므로, 한국 시차(+9시간)를 직접 반영합니다.
# -----------------------------------------------------------------------------
...
# API 요청용(YYYYMMDD)과 화면 표시용(YYYY년 MM월 DD일) 날짜 문자열 생성
...
# -----------------------------------------------------------------------------
# 4. KOBIS API 데이터 호출 함수 (1시간 동안 캐싱 처리)
# @st.cache_data(ttl=3600)을 적용하여 동일한 요청은 3600초(1시간) 동안 API를 재호출하지 않습니다.
# -----------------------------------------------------------------------------
...
if not daily_list:
    ...
    st.info(
        "**확인할 사항:**\n"
        "- 아직 해당 일자의 박스오피스 데이터 집계가 완료되지 않았을 수 있습니다.\n"
        "- 잠시 후 다시 시도해 주세요."
    )
    st.stop()

# -----------------------------------------------------------------------------
# 6. 데이터 전처리 (문자열 -> 숫자 변환 및 정렬)
# -----------------------------------------------------------------------------
df = pd.DataFrame(daily_list)

# 숫자로 변환할 주요 컬럼 지정 (KOBIS API는 모든 숫자 데이터가 문자열로 제공됨)
...
with col1:
    st.metric(label="🎬 1위 영화명", value=top_1["movieNm"])
